...
public class StoreEditorCommentService implements JavaDelegate {
    ...
    public void execute(DelegateExecution delegateExecution) throws Exception {
        ...
        User editorUser = (User) delegateExecution.getVariable("headEditorUser");
        SysUser editor = systemUserService.getSystemUserByUsername(editorUser.getId());
        PublishedBook book = bookService.GetBookByTitle(delegateExecution.getVariable("bookTitle").toString());

        BookComment bookComment = new BookComment(comment,book,editor);
